...
@WebServlet("/servletABMLMedico")
public class servletABMLMedico extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public servletABMLMedico() {
        super();
    }
		...
		if(request.getParameter("btnAgregarMedico") !=null) {
			...
			if(!un.existeUsuario(dni)) {
				...
				if(mn.insertarMedico(cargarMedico(request))) {
					...
					RequestDispatcher rd = request.getRequestDispatcher("/ABMLMedico.jsp");
					rd.forward(request,response);
